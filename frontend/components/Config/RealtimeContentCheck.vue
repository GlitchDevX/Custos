<template>
    <ConfigLayout>
        ...
        <div>
            ...
            <div>
                <FeatureToggle v-model="state.urlCheck" title="URL Check"
                    description="Remove and flag valid URLs or IPs." />

                <FeatureToggle v-model="state.blockedWordsCheck" title="Blocked Words Check"
                    description="Remove and flag banned words. This includes profanity." />

                <UFormField label="Extra Banned Words" size="xl" class="pt-4"
                    :class="{ 'low-opacity': !state.blockedWordsCheck }">
                    <p class="muted-text">
                        List of additional words to ban. You can insert a comma-separated list.
                    </p>
                    <UInputTags :addOnPaste="true" v-model="state.blockedWords" class="mt-1 w-full"
                        :spellcheck="false" />
                </UFormField>
            </div>
        </div>
